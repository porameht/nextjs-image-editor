...
export default class Editor {
  ...
  private _initSketch () {
    ...
    const sketch = new fabric.Rect({
      ...
      fabritor_desc: 'My drawing board by fabritor',
    });
  ...
  public async loadFromJSON (json, errorToast = false) {
    ...
    if (typeof json === 'string') {
      try {
        ...
        console.log(e)
        errorToast && message.error('Loading local template failed, please try again');
        return false;
      }
    }
    if (json[SCHEMA_VERSION_KEY] !== SCHEMA_VERSION) {
      console.warn('This template is no longer compatible with the current version, please change the template');
      return false;
    }
